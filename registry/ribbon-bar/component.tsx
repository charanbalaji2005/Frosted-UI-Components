...
import type { WaveBarProps, CrystalBarProps, ArcBarProps, RibbonBarProps } from "../types";
...
export function WaveBar({
  ...
  fixed = true,
  className,
}: WaveBarProps & { fixed?: boolean }) {
  const [activeId, setActiveId] = useState<string | null>(
    [...leftItems, ...rightItems].find((i) => i.active)?.id ?? leftItems[0]?.id ?? null
  );
  ...
  const panelClass = cn(
    "relative flex flex-col items-center justify-center gap-2 rounded-3xl px-4 py-3",
    glassBase,
    variant === "dark"
      ? "bg-white/[0.07] border border-white/[0.11] shadow-[0_8px_24px_rgba(0,0,0,0.4)]"
  ...
  return (
    <motion.nav
      ...
      aria-label="Wave navigation"
    >
      {}
      <div className={panelClass}>
        {leftItems.map((item) => (
          <WaveIcon key={item.id} item={item}
      ...
      {centerAction && (
        <motion.button
          className={cn(
            ...
            "ring-1 ring-white/20 border border-white/[0.15]",
            glow ? "shadow-[0_0_50px_rgba(120,80,255,0.5),0_8px_32px_rgba(0,0,0,0.5)]"
        ...
        >
          <span className="text-3xl text-white leading-none">{centerAction.icon}</span>
          {}
          {glow && (
            <motion.div
...
function WaveIcon({ item, isActive, onActivate }: {
  item: WaveBarProps["leftItems"][0]; isActive: boolean; onActivate: () => void;
}) {
  return (
    <motion.button
      className={cn(
        "relative flex h-10 w-10 items-center justify-center rounded-2xl text-lg",
        "transition-colors duration-150",
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/30",
        isActive ? "text-purple-300 bg-white/[0.12]" : "text-white/45 hover:text-white/75 hover:bg-white/[0.07]"
      )}
      whileTap={{ scale: 0.88 }}
...
export function CrystalBar({
  ...
  return (
    ...
    >
      {glow && (
        <div className="absolute -bottom-4 left-1/2 h-8 w-2/3 -translate-x-1/2 rounded-full bg-cyan-400/15 blur-xl pointer-events-none" />
      )}
      ...
      <div
        className={cn(
          ...
          glassBase,
          variant === "dark"
            ? "bg-white/[0.07] border-0 shadow-[0_8px_32px_rgba(0,0,0,0.4)]"
            : "bg-black/[0.06]",
          className
        )}
        style={{ clipPath }}
    ...
    >
      <div
        className={cn(
          ...
          glassBase,
          variant === "dark"
            ? "bg-white/[0.07] border border-white/[0.11] shadow-[0_8px_32px_rgba(0,0,0,0.4)]"
            : "bg-black/[0.05] border border-black/[0.08]",
          className
        )}
        style={{
      ...
      >
        ...
        {}
        {glow && (
          <div className="absolute left-[20%] right-[20%] top-0 h-px rounded-full bg-gradient-to-r from-transparent via-blue-300/40 to-transparent pointer-events-none" />
        )}
...
export function RibbonBar({
  ...
  return (
    ...
    >
      ...
      {}
      {glow && (
        <div className="absolute -bottom-4 left-1/2 h-6 w-3/5 -translate-x-1/2 rounded-full bg-amber-500/15 blur-xl pointer-events-none" />
      )}
    </motion.nav>
  );
}

export { WaveBar as default };
